function [TF, ID, ME] = isfunction(FUN)
...
% 3.2 (sept 2015) Modified by JG to return thrown exceptions as well as ID code

if ~iscell(FUN)
    % we use cellfun, so convert to cells
    FUN = {FUN} ;
end
[ID,ME] = cellfun(@local_isfunction,FUN, 'UniformOutput', false) ; % get the identifier for each "function"
ID = ID{:};
ME = ME{:};
...
function [ID,ME] = local_isfunction(FUNNAME)
ME = [];
try    
