...
// Experimental support for asynchronous command transfers
// Send asynchronous command to the device


string MCCDevice::sendAsynchMessage(string message)
{
    try{
        sendAsynchControlTransfer(message);
        return getControlTransfer();
    }
    catch(mcc_err err)
    {
...
void MCCDevice::sendAsynchControlTransfer(string message)
{
    
#ifdef DEBUG
    cout << "Sending Asynch: " << message << "\n";
#endif
    
    // Setup the data buffer
    unsigned char buffer[MAX_MESSAGE_LENGTH+LIBUSB_CONTROL_SETUP_SIZE];
    libusb_fill_control_setup(buffer, // buffer
            LIBUSB_REQUEST_TYPE_VENDOR + LIBUSB_ENDPOINT_OUT, // bmRequestType
            STRINGMESSAGE, //bRequest
            0, //wValue
            0,  //wIndex
            MAX_MESSAGE_LENGTH //wLength
            );
    memcpy(buffer+LIBUSB_CONTROL_SETUP_SIZE,message.data(),message.length());
    
#ifdef DEBUG
    printf("data = ");
    for (int ii=0;ii<LIBUSB_CONTROL_SETUP_SIZE;ii++)
        printf("0x%x ",buffer[ii]);
    printf(" %s\n",buffer+LIBUSB_CONTROL_SETUP_SIZE);
#endif

    // Setup the transfer command
    struct libusb_transfer *transfer_command = libusb_alloc_transfer(0);
    libusb_fill_control_transfer(
            transfer_command,
            pDevHandle,
            buffer, 
            cb_command_out, //libusb_transfer_cb_fn callback
            NULL,
            1000);
    
    // Send the command
    int r = libusb_submit_transfer(transfer_command);
#ifdef DEBUG
    printf("return value from libusb_submit_transfer=%d\n", r);
#endif
//     
//     libusb_free_transfer(transfer_command);
}
