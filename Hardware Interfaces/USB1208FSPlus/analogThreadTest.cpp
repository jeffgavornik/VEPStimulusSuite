...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if ( nrhs != 3 ) mexErrMsgTxt("analogThreadTest: 3 inputs required");
    ...
    if (thePointer == 0){
        mexErrMsgTxt("analogThreadTest: invalid interface");
    }
#ifdef DEBUG
    mexPrintf("analogThreadTest thePointer = 0x%x\n",thePointer);
#endif
...
//             args.usb,args.pulseWidth,args.nPulses);
    ...
    if (pthread_create(pulse_thread, NULL,
            ...
            (void *)&args) != 0) {
        mexErrMsgTxt("analogThreadTest: pthread_create failed");
        *mxGetPr(plhs[0]) = -1;
    }
#else
